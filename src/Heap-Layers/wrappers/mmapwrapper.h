...
#elif defined(_BARE_METAL) // osal

#if 0 // unused in Hoard currently
    static void protect (void * ptr, size_t sz) {
      hl_mmapwrapper_protect (ptr, sz);
    }
    
    static void unprotect (void * ptr, size_t sz) {
      hl_mmapwrapper_unprotect (ptr, sz);
    }
#endif

    static void * map (size_t sz) {
      void * ptr = hl_mmapwrapper_map (sz);
      return  ptr;
    }
  
    static void unmap (void * ptr, size_t sz) {
      hl_mmapwrapper_unmap (ptr, sz);
    }
