...
namespace Hoard {
  ...
  //enum { MAX_MEMORY_PER_TLAB = 2 * 1024 * 1024UL }; // 2MB (Hoard default)
  enum { MAX_MEMORY_PER_TLAB = 1024 * 1024 * 1024UL }; // (Constant for dedup)
  
  /// The maximum number of threads supported (sort of).
  enum { MaxThreads = 2048 };
  
  /// The maximum number of heaps supported.
  //enum { NumHeaps = 128 };  // (Hoard default)
  enum { NumHeaps = 128 }; // (Constant for dedup)
  
  /// Size, in bytes, of the largest object we will cache on a
  /// thread-local allocation buffer.
  //enum { LargestSmallObject = 256UL }; // (Hoard default)
  enum { LargestSmallObject = MAX_MEMORY_PER_TLAB/*65536UL*/ };  // (Constant for dedup)
